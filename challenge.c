...
// =========================================================
void A_to_CSR(
    int rows, int cols, const double* A, int* out_nnz,
    double* values, int* col_indices, int* row_ptrs
) {
    for(int i = 0; i < rows; i++) {
        row_ptrs[i] = *out_nnz;
        for(int j = 0; j < cols; j++) {
            if(A[i * cols + j]!= 0) { // value found
...
void calc_y(
    int rows, const double* values, const int* col_indices,
    int* row_ptrs, const double* x, double* y
) {
    int current_row = 0;
...
void sparse_multiply(
    ...
    A_to_CSR(rows, cols, A, out_nnz, values, col_indices, row_ptrs);
    calc_y(rows, values, col_indices, row_ptrs, x, y);
}
